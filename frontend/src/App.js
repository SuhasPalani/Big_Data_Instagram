...
function App() {
  ...
  const handleSubmit = async (usernames) => {
    try {
      setLoading(true);
      setResults([]);
      ...
      // Update status
      setStatusMessages(prev => [...prev, `Request submitted successfully. Request ID: ${newRequestId}`]);
      
      // Set up WebSocket connection to receive updates
      const ws = new WebSocket(`ws://${window.location.hostname}:8000/ws/${newRequestId}`);
      
      ws.onopen = () => {
        setStatusMessages(prev => [...prev, 'WebSocket connection established']);
      };
      ...
      ws.onerror = (error) => {
        ...
        setStatusMessages(prev => [...prev, `WebSocket error: ${error.message || 'Unknown error'}`]);
        setStatus('error');
        setLoading(false);
      };
    ...
    <div className="container py-5">
      <div className="row mb-4">
        <div className="col">
          ...
          <p className="lead">
            Enter Instagram usernames to analyze their profiles and get detailed analytics.
          </p>
        </div>
      </div>
