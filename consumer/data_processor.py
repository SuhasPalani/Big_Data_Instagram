...
async def process_data(
    instagram_scraper: ApifyClient, usernames: List[str]
) -> List[Dict[str, Any]]:
    try:
        run_input = {"usernames": usernames}
        run = instagram_scraper.actor("apify/instagram-profile-scraper").call(
            run_input=run_input
        )
        ...
        for item in results:
            ...
            if item["followersCount"] > 0 and item["latestPosts"]:
                engagement_rate = (
                    item["latestPosts"][0]["likesCount"]
                    + item["latestPosts"][0]["commentsCount"]
                ) / item["followersCount"]

            if item["latestPosts"]:
                avg_likes = sum(
                    post["likesCount"] for post in item["latestPosts"]
                ) / len(item["latestPosts"])
                avg_comments = sum(
                    post["commentsCount"] for post in item["latestPosts"]
                ) / len(item["latestPosts"])
